...
test("the optional guided-build workflow can resume from the repository state", async () => {
  ...
  assert.equal(state.current_stage, "prepare-submission");
  assert.equal(state.next_command, "prepare-submission");
  assert.equal(state.submission.status, "drafting");
  assert.equal(state.submission.draft_file, "devpost-submission.md");
